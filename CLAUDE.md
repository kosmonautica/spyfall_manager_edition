...
**Scenario overview sheet:** DIN A4 portrait (2480×3508 px at 300 dpi) — title at top, one card per scenario (Manager card excluded) in a grid. Always fits on exactly one page so all scenarios are visible without turning pages. Layout adapts automatically to the number of scenarios:
